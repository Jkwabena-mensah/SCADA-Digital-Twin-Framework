...
- **Details**:
  ...
  - Defined a `SensorReading` data model in `src/WebController/WebController/Models/SensorReading.cs` to store sensor data (Id, AssetId, Timestamp, MotorAmps, Temperature, Vibration, Status), with AssetId and Status updated to nullable fields.
  - Created an `ApplicationDbContext` in `src/WebController/WebController/Data/ApplicationDbContext.cs` to manage the database, registered with a temporary in-memory database in `Program.cs`.
...
- **Future**: Configure a SQL Server database, create migrations, and integrate with the Python simulator via MQTT.
